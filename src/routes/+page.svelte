<script lang="ts">
	...
	import { initAuth, getToken } from '$lib/auth.svelte';
	import { formatTimeAgo } from '$lib/format';
	import { getRecentAnalyses } from '$lib/storage';
	import type { RecentAnalysis } from '$lib/storage';
	import { VERDICT_COLORS, VERDICT_LABELS } from '$lib/verdict';
	import SymbolSearch from '../components/SymbolSearch.svelte';

	// Landing: hero + search + recent analyses. Deep links (?symbol=X)
	// redirect to the dashboard routes (old bookmark compat).
	...
	onMount(() => {
		...
		if (q) {
			...
			goto(m === 'options' ? `/options/${sym}` : `/s/${sym}`, { replaceState: true });
		}
	});
		...
		{:else}
			<div class="flex flex-col gap-2">
				{#each recent as r}
					{@const color =
						r.verdict !== 'pending'
							? (VERDICT_COLORS[r.verdict as keyof typeof VERDICT_COLORS] ?? '#9ca3af')
							: '#9ca3af'}
					{@const label =
						r.verdict !== 'pending'
							? (VERDICT_LABELS[r.verdict as keyof typeof VERDICT_LABELS] ?? 'PENDING')
							: 'PENDING'}
					<a
						href={`/s/${r.symbol}`}
